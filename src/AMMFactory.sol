...
contract AMMFactory {
    ...
    event PairCreated(
        address indexed tokenA, address indexed tokenB, address pair, uint256 pairIndex, bool deterministicCreate2
    );
    ...
    function createPair2(address tokenA, address tokenB, bytes32 salt) external returns (address pair) {
        ...
        bytes memory bytecode = abi.encodePacked(type(AMM).creationCode, abi.encode(token0, token1, owner));

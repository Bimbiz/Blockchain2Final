...
contract LPPositionNFT is ERC721Enumerable, AccessControl {
    ...
    event PositionMinted(uint256 indexed tokenId, address indexed owner, address pair, uint256 lpAmount);
    event PositionBurned(uint256 indexed tokenId, address indexed owner);

    error NotOwnerOrApproved();

    constructor(address admin, string memory baseURI) ERC721("LP Position", "LPP") {
    ...
    function mintPosition(address to, address pair, uint256 lpAmount)
        external
        onlyRole(MINTER_ROLE)
        returns (uint256 tokenId)
    {
        tokenId = _nextTokenId++;
        positions[tokenId] = PositionInfo({ pair: pair, lpAmount: lpAmount, mintedAt: block.timestamp });

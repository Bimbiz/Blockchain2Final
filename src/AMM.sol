...
contract AMM is ERC20, Ownable, ReentrancyGuard, Pausable {
    ...
    constructor(address _tokenA, address _tokenB, address _owner) ERC20("AMM LP Token", "ALP") Ownable(_owner) {
        ...
        returns (uint256 amountA, uint256 amountB, uint256 liquidity)
    {
        ...
        if (totalSupply_ == 0) {
            ...
            liquidity = _min((amountA * totalSupply_) / _reserveA, (amountB * totalSupply_) / _reserveB);
        }
    ...
    function quote(uint256 amountA_, uint256 _reserveA, uint256 _reserveB) public pure returns (uint256 amountB_) {
    ...
    function _verifyKInvariant(uint256 newReserveIn, uint256 newReserveOut, uint256 oldReserveIn, uint256 oldReserveOut)
        internal
        pure
    {
        assembly {
